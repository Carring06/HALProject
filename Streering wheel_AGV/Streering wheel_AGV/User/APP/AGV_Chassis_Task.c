...
fp32 DJI_3508_speed_pid[3] = {13.0f, 0.03f, 0.0f};		//底盘3508电机速度环PID参数 {Kp, Ki, Kd}
...
void DM_Motor_Four_Ctrl(fp32 tor1, fp32 tor2, fp32 tor3, fp32 tor4)
{
	...
	mit_ctrl2(&hfdcan1,Chassis_6220_DM_Motor[0].ID_Set.TxIdentifier,0,0,0,0,tor1,1);
	mit_ctrl2(&hfdcan2,Chassis_6220_DM_Motor[1].ID_Set.TxIdentifier,0,0,0,0,tor2,1);
	mit_ctrl2(&hfdcan2,Chassis_6220_DM_Motor[2].ID_Set.TxIdentifier,0,0,0,0,tor3,1);
	mit_ctrl2(&hfdcan1,Chassis_6220_DM_Motor[3].ID_Set.TxIdentifier,0,0,0,0,tor4,1);
}
...
{
    wheel_angle_last[0]= 0;
    wheel_angle_last[1]= 0;
    wheel_angle_last[2]= 0;
    wheel_angle_last[3]= 0;		
}
//2.3.4号电机pid参数
...
/**
  ...
  * @attention
  */
int8_t motor_sign=1;//电机正反转标志位

float   x;

int8_t sign_group[4]={1,1,1,1};
 int16_t wheel_speed[4];
void AGV_speed_calc(Chassis_Speed *speed, int16_t* out_speed) 
{
	  //3508目标速度计算
   
    fp32 wheel_rpm_ratio;   //线速度---rpm 转换,其实这样的话也行,你不用去转化太多东西,转化得多错的多
...
void CHASSIS_Single_Loop_Out(void)
{
    ...
    /************************************将控制指令发送给电机*********************************************/
		...
		//can发送给3508
		//DJI_Motor_ctrl(chassis_3508_motor, &hfdcan1,0); //底盘3508电机调控
