...
{
    static uint8_t *USB_Current_Buf;
    /* 切换当前接收缓冲区 */ 
    if(USB_Current_Buf == USB_RxBuf[1]) {
        USB_Current_Buf = USB_RxBuf[0];
    }
    else {
        USB_Current_Buf = USB_RxBuf[1];
    }
    USBD_CDC_SetRxBuffer(&hUsbDeviceHS, USB_Current_Buf);
    USBD_CDC_ReceivePacket(&hUsbDeviceHS);
    /* 处理接收到的数据 */
    if (USB_Current_Buf != NULL)
    {
        // user_code
    }
}
